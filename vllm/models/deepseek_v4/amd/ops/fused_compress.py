...
from vllm.models.deepseek_v4.amd.ops.compress_plan import CompressPlan
...
# [vLLM-DSV4-ATOM-port] Split the single try-block into two independent imports.
# The HCA 2-kernel module hard-requires flydsl>=0.1.8 (tensor_shim STensor needs
# flydsl.compiler.protocol.extract_to_ir_values, absent on the pinned 0.1.4.2);
# importing it in the SAME try as the CSA kernel would null BOTH on the HCA
# failure, forcing all-Triton compress. Keeping them separate lets the CSA/Indexer
# flydsl fast path load while HCA falls back to the ATOM Triton kernel.
try:
...
def fused_compress_attn(
    *,
    ...
    if _flydsl_mode == "always" and not _flydsl_shape_ok:
        raise RuntimeError(
            f"DSV4_ROCM_FUSED_COMPRESS_USE_FLYDSL=always but shape "
