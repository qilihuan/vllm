...
# the `safetensors.torch.load(bytes)` path used when `DSV4_ROCM_DISABLE_MMAP=true`
...
from vllm.models.deepseek_v4.amd.model_loader.weight_utils import (
    download_weights_from_hf,
    filter_duplicate_safetensors_files,
)
from vllm.models.deepseek_v4.amd.model_ops.base_config import QuantizeMethodBase
from vllm.models.deepseek_v4.amd.model_ops.fused_moe.layer import FusedMoEMethodBase
...
from vllm.models.deepseek_v4.amd.plugin.prepare import is_sglang
...
def load_model(
    ...
    def should_fuse_shared_expert_weight(name: str, matching_name: str) -> bool:
        ...
        model_quant_config = getattr(
            getattr(model, "dsv4_config", None), "quant_config", None
        )
    ...
    if weights_mapper is None:
        model_mapper = getattr(model, "hf_to_dsv4_mapper", None)
    ...
    for module_name, module in model.named_modules():
        if hasattr(module, "process_weights_after_loading"):
            # vLLM's Attention/MLAAttention post-load hooks require an
            # ``act_dtype`` (no default); this loader's own modules take no
            # argument. Pass the model compute dtype when the hook requires it,
            # rather than patching vLLM's Attention class. (Ported from ATOM.)
            _post_load = module.process_weights_after_loading
            try:
                _params = inspect.signature(_post_load).parameters
                _needs_act_dtype = (
                    "act_dtype" in _params
                    and _params["act_dtype"].default is inspect._empty
                )
            except (TypeError, ValueError):
                _needs_act_dtype = False
            if _needs_act_dtype:
                _post_load(torch.bfloat16)
            else:
                _post_load()
        ...
        if isinstance(quant_method, QuantizeMethodBase) and True:
