...
from vllm.models.deepseek_v4.amd.quant_spec import (
    LayerQuantConfig,
    get_quant_parser,
)
from vllm.models.deepseek_v4.amd.utils import get_open_port
from vllm.models.deepseek_v4.amd.utils.distributed.utils import stateless_init_torch_distributed_process_group
from torch.distributed import ProcessGroup, ReduceOp
from transformers import AutoConfig, GenerationConfig, PretrainedConfig

# plugin-related utilities
from vllm.models.deepseek_v4.amd.plugin import is_plugin_mode, is_vllm
from vllm.models.deepseek_v4.amd.plugin.config import PluginConfig
    ...
    def set_splitting_ops_for_v1(self):
        ...
        if self.splitting_ops is None:
            self.splitting_ops = [
                ...
                "aiter.mla_attention",
                "aiter.dsv4_vllm_mha_attention",
                "aiter.dsv4_vllm_mla_attention",
            ]
...
class ParallelConfig:
    ...
    def __post_init__(self) -> None:
        # Data-parallel fields come from the vLLM parallel config; the former
        # DSV4_ROCM_DP_* env overrides were removed.
        pass
...
def _normalize_moe_config_fields(
    ...
    if existing_n_shared is not None:
        ...
        from vllm.models.deepseek_v4.amd.models.utils import ckpt_shared_expert_count
...
class KVEventsConfig:
    ...
    def from_env(cls) -> "KVEventsConfig":
        """Build a config from `DSV4_ROCM_KV_EVENTS_*` env vars. Provides an env-only
...
class Config:
    ...
    runner_qualname: str = "vllm.models.deepseek_v4.amd.model_engine.model_runner.ModelRunner"
...
_current_dsv4_config: Optional[Config] = None


def set_current_dsv4_config(dsv4_config: Config):
    global _current_dsv4_config
    _current_dsv4_config = dsv4_config


def _get_current_dsv4_config_from_vllm_forward_context() -> Optional[Config]:
    ...
    if not is_forward_context_available():
        return None
    try:
        return get_vllm_forward_context().additional_kwargs.get("dsv4_config")
    except Exception:
        return None


def get_current_dsv4_config() -> Config:
    # Try to get the atom config from forward context first in vLLM plugin mode.
    forward_dsv4_config = _get_current_dsv4_config_from_vllm_forward_context()
    if forward_dsv4_config is not None:
        return forward_dsv4_config
    assert _current_dsv4_config is not None, "Current atom config is not set"
    return _current_dsv4_config


@contextmanager
def use_custom_dsv4_config(custom_dsv4_config: Config):
    # Temporarily masquerade the custom dsv4_config as the current dsv4_config
    # for the current context and restore upon exit
    global _current_dsv4_config
    prev = _current_dsv4_config
    _current_dsv4_config = custom_dsv4_config
    try:
        yield custom_dsv4_config
    finally:
        _current_dsv4_config = prev
