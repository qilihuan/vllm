...
import numpy as np
import torch
from vllm.models.deepseek_v4.amd.config import Config, KVCacheTensor, ParallelConfig
...
def set_forward_context(
    ...
    dp_metadata: Optional[DPMetadata] = None
    if dsv4_config.parallel_config.data_parallel_size > 1 and num_tokens is not None:
        dp_metadata = DPMetadata.make(
            dsv4_config.parallel_config,
            # attn_metadata,
            num_tokens or 0,
    ...
    _forward_context = ForwardContext(
        ...
        no_compile_layers=dsv4_config.compilation_config.static_forward_context,
    ...
    # _forward_context.no_compile_layers = dsv4_config.compilation_config.static_forward_context
    # _forward_context = ForwardContext(no_compile_layers=dsv4_config.compilation_config.static_forward_context, attn_metadata=attn_metadata)
...
_logger = logging.getLogger("vllm.models.deepseek_v4.amd")
...
def get_kvconnector(role: str = "worker", config: Optional[Config] = None) -> Any:
    ...
    if role == "worker":
        ...
        if _global_kvconnector is None:
            from vllm.models.deepseek_v4.amd.kv_transfer.disaggregation import KVConnectorFactory
