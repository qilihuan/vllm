...
def _build_dsv4_speculative_config_from_vllm(vllm_spec_config: Any):
    ...
    the draft hf_config via dsv4_config.speculative_config.
    ...
    from vllm.models.deepseek_v4.amd.config import SpeculativeConfig
...
def _generate_dsv4_config_from_vllm_config(config: Any) -> PluginConfig:
    from vllm.models.deepseek_v4.amd.config import Config, CompilationConfig
    ...
    dsv4_speculative_config = _build_dsv4_speculative_config_from_vllm(
...
def _generate_dsv4_config_from_sglang_config(config: Any):
    ...
    from vllm.models.deepseek_v4.amd.config import Config, ParallelConfig, CompilationConfig
    ...
    (
        ...
        dsv4_data_parallel_size,
        dsv4_data_parallel_rank,
    ...
    sgl_parallel_config = ParallelConfig(
        data_parallel_size=dsv4_data_parallel_size,
        data_parallel_size_local=dsv4_data_parallel_size,
        data_parallel_rank=dsv4_data_parallel_rank,
        data_parallel_rank_local=dsv4_data_parallel_rank,
    )
    ...
    return Config(
        ...
        tensor_parallel_size=dsv4_tensor_parallel_size,
...
def generate_dsv4_config_for_plugin_mode(config: Any = None):
    """
    ...
    logger.info("Generate config for plugin mode from passed config")
    from vllm.models.deepseek_v4.amd.config import set_current_dsv4_config

    dsv4_config = _generate_dsv4_config_from_vllm_config(config)

    # set the current atom config for the custom model
    set_current_dsv4_config(dsv4_config)
