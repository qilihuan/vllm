...
from vllm.models.deepseek_v4.amd.utils.block_convert import kv_indices_generate_triton
...
class AiterMlaSparseMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        for layer_name in layer_names or []:
            ...
            owner_dsv4_config = getattr(indexer_cache, "dsv4_config", None)
            sfc = (
                owner_dsv4_config.compilation_config.static_forward_context
                if owner_dsv4_config is not None
                else default_sfc
            )
            ...
            if indexer is None or sparse_attn is None:
                logger.warning(
                    ...
                    "(indexer=%s, sparse_attn=%s, owner_dsv4_config=%s)",
                    attention_prefix,
                    indexer is not None,
                    sparse_attn is not None,
                    owner_dsv4_config is not None,
                )
...
class AiterMlaSparseIndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        from vllm.models.deepseek_v4.amd.models.utils import extract_layer_index
