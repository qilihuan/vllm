...
DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME = "model.layers.0.deepseek_v4_proxy"
DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE = 128
...
def _classical_block_bytes(hf_config) -> int:
    ...
    csa_main = (DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE // 4) * head_dim * 2
    csa_index = (DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE // 4) * index_dim
    hca_main = (DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE // 128) * head_dim * 2
...
def _proxy_page_bytes(vllm_config) -> int:
    ...
    min_blocks = max(
        1,
        (max_model_len + DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE - 1)
        // DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE,
    )
...
def slice_deepseek_v4_proxy_cache_views(
    ...
    *,
    ...
    for ratio in ratios:
        ...
        swa.append(swa_view)
        if ratio == 4:
            k = DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE // 4
            ...
            k = DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE // 128
            main_bytes = num_blocks * k * head_dim * 2
            main = (
                take_bytes(main_bytes)
                .view(torch.bfloat16)
...
class DeepseekV4ProxyMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        return self._build_and_attach_dsv4_md(common_attn_metadata, capturing=False)
    ...
    def build_for_cudagraph_capture(self, common_attn_metadata):
        ...
        return self._build_and_attach_dsv4_md(common_attn_metadata, capturing=True)

    def _build_and_attach_dsv4_md(self, common_attn_metadata, *, capturing):
        ...
        Returns the same ``common_attn_metadata`` (now carrying ``dsv4_md``)
        ...
        proxy = sfc.get(DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME)
        model = getattr(proxy, "_dsv4_model", None) if proxy is not None else None
        meta_params = getattr(model, "_dsv4_meta_params", None)
        if model is None or meta_params is None:
            ...
            # dsv4_md and falls back to an inline eager build (force_dummy).
            return common_attn_metadata
        slot_allocator = (
            None if capturing else getattr(model, "_dsv4_slot_allocator", None)
        )
        decode_bufs = getattr(model, "_dsv4_decode_bufs", None)
        md = build_dsv4_attention_metadata(
        ...
        common_attn_metadata.dsv4_md = md
        return common_attn_metadata


class DeepseekV4ProxyBackend(AttentionBackend):
    forward_includes_kv_cache_update = True

    @staticmethod
    def get_name() -> str:
        return "DSV4_ROCM_DEEPSEEK_V4_PROXY"

    @staticmethod
    def get_supported_kernel_block_sizes():
        return [DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE]
    ...
    @staticmethod
    def get_builder_cls():
        return DeepseekV4ProxyMetadataBuilder
...
class DeepseekV4ProxyAttention(nn.Module, AttentionLayerBase):
    def __init__(self, prefix: str = DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME):
        super().__init__()
        self.prefix = prefix
    ...
    def get_attn_backend(self) -> type[AttentionBackend]:
        return DeepseekV4ProxyBackend

    def get_kv_cache_spec(self, vllm_config) -> KVCacheSpec:
        page_bytes = _proxy_page_bytes(vllm_config)
        head_size = (page_bytes + 2 * DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE - 1) // (
            2 * DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE
        )
        return FullAttentionSpec(
            block_size=DSV4_ROCM_DEEPSEEK_V4_BLOCK_SIZE,
            num_kv_heads=1,
            head_size=head_size,
            dtype=torch.uint8,
        )
...
def register_deepseek_v4_proxy_layer(vllm_config) -> DeepseekV4ProxyAttention:
    sfc = vllm_config.compilation_config.static_forward_context
    existing = sfc.get(DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME)
    if isinstance(existing, DeepseekV4ProxyAttention):
        return existing
    if existing is not None:
        raise ValueError(f"Duplicate layer name: {DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME}")
    proxy = DeepseekV4ProxyAttention()
    sfc[DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME] = proxy
    return proxy
...
class _V4DecodeMetaBuffers:
    ...
    def __init__(
        ...
        *,
        ...
        from vllm.models.deepseek_v4.amd.utils import CpuGpuBuffer
        ...
        from vllm.models.deepseek_v4.amd.ops.compress_plan import (  # noqa: F401
...
def bind_deepseek_v4_proxy_cache_views(model, vllm_config) -> bool:
    sfc = vllm_config.compilation_config.static_forward_context
    proxy = sfc.get(DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME)
    if proxy is None or not isinstance(proxy, DeepseekV4ProxyAttention):
        return False
    if not isinstance(proxy.kv_cache, torch.Tensor) or proxy.kv_cache.numel() == 0:
        return False
    ptr = proxy.kv_cache.untyped_storage().data_ptr()
    if getattr(model, "_dsv4_vllm_proxy_cache_ptr", None) == ptr:
        return True
    ...
    if not hasattr(model, "_dsv4_slot_allocator"):
        model._dsv4_slot_allocator = _V4StateSlotAllocator(num_slots)
    model._dsv4_meta_params = SimpleNamespace(
        num_slots=num_slots,
    ...
    proxy._dsv4_model = model
    if not hasattr(model, "_dsv4_decode_bufs"):
        ...
        model._dsv4_decode_bufs = _V4DecodeMetaBuffers(
            num_slots=num_slots,
    ...
    model._dsv4_vllm_proxy_cache_ptr = ptr
    return True
...
def _infer_dsv4_attn_state(common_attn_metadata):
    from vllm.models.deepseek_v4.amd.utils.forward_context import AttnState
...
def _make_compress_plans(
    ...
    from vllm.models.deepseek_v4.amd.ops import make_compress_plans
    from vllm.models.deepseek_v4.amd.utils import CpuGpuBuffer
...
def build_dsv4_attention_metadata(
    common_attn_metadata,
    *,
    meta_params=None,
    slot_allocator=None,
    ...
    """
    from vllm.models.deepseek_v4.amd.utils.forward_context import AttentionMetaData

    if common_attn_metadata is None:
        return AttentionMetaData()
    state = _infer_dsv4_attn_state(common_attn_metadata)
...
def _make_decode_compress_plans(extend_lens_cpu, context_lens_cpu, bufs):
    ...
    from vllm.models.deepseek_v4.amd.ops.compress_plan import make_compress_plans
...
def _populate_decode_persistent(
    ...
    from vllm.models.deepseek_v4.amd.plugin.vllm.deepseek_v4_ops import write_v4_decode_hca_compress_tail
...
def _populate_prefill(md, common, batch_np, pos_np, q_np, positions_gpu):
    ...
    from vllm.models.deepseek_v4.amd.ops import write_v4_paged_prefill_indices
...
def _populate_decode(md, common, batch_np, pos_np, positions_gpu):
    ...
    from vllm.models.deepseek_v4.amd.ops import write_v4_paged_decode_indices

    from vllm.models.deepseek_v4.amd.plugin.vllm.deepseek_v4_ops import (
        write_v4_decode_hca_compress_tail,
    )
...
def get_deepseek_v4_proxy_metadata_from_vllm_context():
    ...
    if isinstance(meta, dict):
        return meta.get(DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME)
    if isinstance(meta, list) and meta and isinstance(meta[0], dict):
        return meta[0].get(DSV4_ROCM_DEEPSEEK_V4_PROXY_LAYER_NAME)
    return None


@contextmanager
def deepseek_v4_forward_context(
    *,
    dsv4_config,
    input_ids,
    positions,
    ...
    from vllm.models.deepseek_v4.amd.utils.forward_context import (
        Context,
    ...
    attn_metadata = getattr(common_attn_metadata, "dsv4_md", None)
    if attn_metadata is None:
        ...
        attn_metadata = build_dsv4_attention_metadata(
            common_attn_metadata,
            meta_params=meta_params,
            slot_allocator=slot_allocator,
        )
