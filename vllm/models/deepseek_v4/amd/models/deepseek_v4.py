...
if TYPE_CHECKING:
    from vllm.models.deepseek_v4.amd.model_ops.attentions.deepseek_v4_attn import AttentionMetaData_DSV4
...
from vllm.models.deepseek_v4.amd.model_loader.loader import WeightsMapper
...
from vllm.models.deepseek_v4.amd.model_ops import module_dispatch_ops as _module_dispatch_ops  # noqa: F401
from vllm.models.deepseek_v4.amd.model_ops.embed_head import ParallelLMHead, VocabParallelEmbedding
from vllm.models.deepseek_v4.amd.model_ops.layernorm import RMSNorm, rmsnorm2d_fwd_
from vllm.models.deepseek_v4.amd.model_ops.linear import (
...
from vllm.models.deepseek_v4.amd.model_ops.fused_moe.layer import FusedMoE
from vllm.models.deepseek_v4.amd.model_ops.quant_v4 import act_quant_inplace
from vllm.models.deepseek_v4.amd.model_ops.sparse_attn_v4 import (
    hc_split_sinkhorn,
)
from vllm.models.deepseek_v4.amd.model_ops.topK import (
    is_rocm_aiter_fusion_shared_expert_enabled_for_quant_config,
)
from vllm.models.deepseek_v4.amd.model_ops.triton_hash_topk import hash_topk_triton
from vllm.models.deepseek_v4.amd.model_ops.triton_rmsnorm_nw import rmsnorm_nw
from vllm.models.deepseek_v4.amd.model_ops.utils import dsv4_parameter
from vllm.models.deepseek_v4.amd.ops import (
    CompressPlan,
    csa_translate_pack,
    fused_compress_attn,
...
from vllm.models.deepseek_v4.amd.utils.decorators import support_torch_compile
...
class Indexer(nn.Module):
    ...
    def __init__(self, args: DeepseekV4Args, compress_ratio: int = 4, prefix: str = ""):
        ...
        get_current_dsv4_config().compilation_config.static_forward_context[
            prefix
        ] = self
...
class DeepseekV4Attention(nn.Module):
    ...
    def __init__(
        ...
        dsv4_config = get_current_dsv4_config()
        dsv4_config.compilation_config.static_forward_context[self.layer_name] = self
    ...
    def process_weights_after_loading(self) -> None:
        ...
        self.wo_a.weight = dsv4_parameter(bf16)
        try:
...
class MoE(nn.Module):
    ...
    def _gather_ids_for_dp(ids: torch.Tensor, ctx) -> torch.Tensor:
        ...
        if dp_eager_mode:
            from vllm.models.deepseek_v4.amd.model_ops.fused_moe.layer import all_gatherv

            sizes = ctx.dp_metadata.get_sizes_across_dp()
            ids_2d = all_gatherv(ids_2d, sizes, get_dp_group())
        else:
            from vllm.models.deepseek_v4.amd.model_ops.fused_moe.layer import all_gather_with_padding
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.hc_attn_fn = dsv4_parameter(
            torch.empty(mix_hc, hc_dim, dtype=torch.float32)
        )
        self.hc_ffn_fn = dsv4_parameter(
            torch.empty(mix_hc, hc_dim, dtype=torch.float32)
        )
        self.hc_attn_base = dsv4_parameter(torch.empty(mix_hc, dtype=torch.float32))
        self.hc_ffn_base = dsv4_parameter(torch.empty(mix_hc, dtype=torch.float32))
        self.hc_attn_scale = dsv4_parameter(torch.empty(3, dtype=torch.float32))
        self.hc_ffn_scale = dsv4_parameter(torch.empty(3, dtype=torch.float32))
...
class DeepseekV4Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        hc_dim = hc_mult * args.dim
        self.hc_head_fn = dsv4_parameter(
            torch.empty(hc_mult, hc_dim, dtype=torch.float32)
        )
        self.hc_head_base = dsv4_parameter(torch.empty(hc_mult, dtype=torch.float32))
        self.hc_head_scale = dsv4_parameter(torch.empty(1, dtype=torch.float32))
...
class DeepseekV4ForCausalLM(nn.Module):
    ...
    def __init__(self, config: Config, prefix: str = "") -> None:
        ...
        self.dsv4_config.quant_config = self.args.quant_config
        self.model = DeepseekV4Model(dsv4_config=config, args=self.args)
