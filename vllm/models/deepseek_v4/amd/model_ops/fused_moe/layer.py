...
from vllm.models.deepseek_v4.amd.config import (
    Config,
    QuantizationConfig,
    get_current_dsv4_config,
)
from vllm.models.deepseek_v4.amd.model_loader.weight_utils import set_weight_attrs
from vllm.models.deepseek_v4.amd.model_ops.base_config import QuantizeMethodBase
from vllm.models.deepseek_v4.amd.model_ops.fused_moe.config import (
...
from vllm.models.deepseek_v4.amd.model_ops.fused_moe.modular_kernel import (
    FusedMoEModularKernel,
    FusedMoEPrepareAndFinalize,
)
from vllm.models.deepseek_v4.amd.model_ops.fused_moe.mori_prepare_finalize import MoriPrepareAndFinalize
from vllm.models.deepseek_v4.amd.model_ops.topK import (
    init_aiter_topK_meta_data,
    is_rocm_aiter_fuse_routed_scaling_factor,
    is_rocm_aiter_fusion_shared_expert_enabled_for_quant_config,
)
from vllm.models.deepseek_v4.amd.model_ops.topK import rocm_aiter_grouped_topk as grouped_topk
from vllm.models.deepseek_v4.amd.model_ops.topK import rocm_aiter_topk_softmax as fused_topk
from vllm.models.deepseek_v4.amd.model_ops.utils import (
    _has_module,
    dsv4_parameter,
...
from vllm.models.deepseek_v4.amd.plugin.vllm.moe import FusedMoEDecoratorForPluginMode
from vllm.models.deepseek_v4.amd.quant_spec import LayerQuantConfig, should_skip_online_quant
from vllm.models.deepseek_v4.amd.quantization.quark.utils import weight_dequant_fp8
from vllm.models.deepseek_v4.amd.utils.custom_register import direct_register_custom_op
from vllm.models.deepseek_v4.amd.utils.decorators import mark_trace
from vllm.models.deepseek_v4.amd.utils.forward_context import get_forward_context
...
class FusedMoEParallelConfig:
    ...
    def make(
        ...
        return FusedMoEParallelConfig(
            ...
            ep_rank=ep_rank,
            use_ep=True,
            local_ep_size=dsv4_config.parallel_config.data_parallel_size_local
            * tp_size_,
        )
...
class FusedMoEMethodBase(QuantizeMethodBase):
    ...
    def _maybe_make_prepare_finalize(
        ...
        if moe.use_mori_kernels:
            ...
            from vllm.models.deepseek_v4.amd.utils.tbo.ubatching import tbo_enabled

            handle = all2all_manager.get_handle(all_to_all_args)
            is_async = tbo_enabled()
            dsv4_config = get_current_dsv4_config()
            low_latency = getattr(dsv4_config, "enable_low_latency", False)
            ...
            if is_async:
                from vllm.models.deepseek_v4.amd.model_ops.fused_moe.mori_prepare_finalize import (
                    _NUM_TBO_UBATCHES,
                    init_mori_op,
                )
...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.w13_weight = dsv4_parameter(self._maybe_pad_weight(layer.w13_weight.data))
        layer.w2_weight = dsv4_parameter(self._maybe_pad_weight(layer.w2_weight.data))
...
class Mxfp4MoEMethod(FusedMoEMethodBase):
    def __init__(self, quant_config: LayerQuantConfig, moe: FusedMoEConfig):
        ...
        self.use_triton = (
            ...
            or (gfx.startswith("gfx95") and False)
        )
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        w13_weight_scale = dsv4_parameter(
            torch.zeros(
                num_experts,
        ...
        w2_weight_scale = dsv4_parameter(
            torch.zeros(
                num_experts,
                hidden_size,
        ...
        if self.static_input_scales:
            w13_input_scale = dsv4_parameter(
                torch.ones(num_experts, dtype=torch.float32)
            )
            layer.register_parameter("w13_input_scale", w13_input_scale)
            set_weight_attrs(w13_input_scale, extra_weight_attrs)

            w2_input_scale = dsv4_parameter(
                torch.ones(num_experts, dtype=torch.float32)
            )
    ...
    def process_weights_after_loading(self, layer):
        ...
        if self.use_triton:
            from vllm.models.deepseek_v4.amd.config import get_current_dsv4_config
            from vllm.models.deepseek_v4.amd.model_ops.fused_moe.triton_kernels import _swizzle_mxfp4
            ...
            (
                ...
                dsv4_config.tensor_parallel_size,
        ...
        layer.w13_weight_scale = dsv4_parameter(shuffled_w13_scale)
        layer.w2_weight_scale = dsv4_parameter(shuffled_w2_scale)
    ...
    def apply(
        ...
        if self.use_triton:
            from vllm.models.deepseek_v4.amd.model_ops.fused_moe.triton_kernels import (
...
class CompressedTensorsFp8MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if not self.per_channel and not self.block_quant:
            ...
            layer.w13_weight_scale = dsv4_parameter(max_w13_scales)
    ...
    def get_fused_moe_quant_config(
        ...
        from vllm.models.deepseek_v4.amd.model_ops.fused_moe.config import fp8_w8a8_moe_quant_config
...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def _normalize_weights_and_scales(self, layer: nn.Module):
        ...
        layer.w13_weight = dsv4_parameter(w13_weight)
        layer.w13_weight_scale = dsv4_parameter(w13_weight_scale)
        layer.w2_weight = dsv4_parameter(w2_weight)
        layer.w2_weight_scale = dsv4_parameter(w2_weight_scale)
        if w13_input_scale is not None:
            layer.w13_input_scale = dsv4_parameter(w13_input_scale)
        if w2_input_scale is not None:
            layer.w2_input_scale = dsv4_parameter(w2_input_scale)
    ...
    def _process_block_quant(self, layer: nn.Module) -> None:
        ...
        if not self.need_normalize_e4m3fn_to_e4m3fnuz:
            layer.w13_weight = dsv4_parameter(layer.w13_weight.data)
            layer.w13_weight_scale = dsv4_parameter(layer.w13_weight_scale.data)
            layer.w2_weight = dsv4_parameter(layer.w2_weight.data)
            layer.w2_weight_scale = dsv4_parameter(layer.w2_weight_scale.data)
    ...
    def _process_tensor_quant(self, layer: nn.Module) -> None:
        if not self.quant_config.is_dynamic:
            ...
            layer.w13_input_scale = dsv4_parameter(layer.w13_input_scale.max())
            layer.w2_input_scale = dsv4_parameter(layer.w2_input_scale.max())
...
def moe_forward(
    ...
    dsv4_config = get_current_dsv4_config()
    self = dsv4_config.compilation_config.static_forward_context[layer_name]
...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        if fuse_shared_experts and self.num_fused_shared_experts > 0:
            init_aiter_topK_meta_data(
                ...
                max_num_tokens=dsv4_config.max_num_batched_tokens,
                is_EP=self.use_ep,
            )
        ...
        moe = FusedMoEConfig(
            ...
            in_dtype=dsv4_config.torch_dtype,
    ...
    def select_experts(
        ...
        if use_grouped_topk:
            ...
            if scoring_func == "softmax":
                ...
                if fuse_shared:
                    import vllm.models.deepseek_v4.amd.model_ops.topK as _topK_mod
    ...
    def forward_impl_graph(
        ...
        use_dp_gather_scatter = (
            ...
            and get_current_dsv4_config().enable_dp_attention
        )
        if use_dp_gather_scatter:
            ...
            from vllm.models.deepseek_v4.amd.utils.tbo.ubatching import tbo_active

            _tbo = tbo_active()
            if _tbo:
                from vllm.models.deepseek_v4.amd.utils.tbo.ubatching import (
