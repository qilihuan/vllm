...
from vllm.models.deepseek_v4.amd.config import QuantizationConfig, get_current_dsv4_config
...
from vllm.models.deepseek_v4.amd.model_ops.utils import MXFP4_QUANT_BLOCK_SIZE  # noqa
...
def gemm_a4w4_quant(
    ...
    if params_dtype == dtypes.fp4x2 and use_fp4_non_shuffle_triton_gemm():
        if gemm_afp4wfp4 is None:
            raise RuntimeError(
                "DSV4_ROCM_USE_FP4_NON_SHUFFLE_TRITON_GEMM=1 requires aiter.ops.triton.gemm_afp4wfp4"
            )
        if x_scale is None:
    ...
    # Preshuffle FP4 Triton path: used when DSV4_ROCM_USE_TRITON_GEMM is enabled
...
class LinearBase(nn.Module):
    def __init__(
        ...
        if self.source_quant_dtype is not None:
            ...
            self.weight = dsv4_parameter(torch.empty(weight_size, dtype=params_dtype))
        if bias:
            output_type = get_current_dsv4_config().torch_dtype
            self.bias = dsv4_parameter(torch.empty(self.output_size, dtype=output_type))
        ...
        if quant_type != QuantType.No and self.source_quant_dtype is None:
            if quant_type == QuantType.per_Tensor:
                ...
                if not layer_quant_config.is_dynamic:
                    self.input_scale = dsv4_parameter(
                        torch.empty(
    ...
    def process_weights_after_loading(self):
        ...
        if (
            ...
            self.weight_scale = dsv4_parameter(w_s)
